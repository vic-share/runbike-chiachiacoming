...
export const handleRaces = async ({ request, env, ctx, url, path, method, getDB, TEAM_ID, corsHeaders }) => {
    ...
    if (path === "/api/race-events") {
        if (method === "GET") { 
            const events = await getDB().prepare(`SELECT id, team_id, series_id, date, name, location, public_url as url FROM RaceEvents WHERE team_id = ${TEAM_ID} ORDER BY date DESC`).all(); 
