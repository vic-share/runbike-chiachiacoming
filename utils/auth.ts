...
export const PERMISSIONS = {
  ...
  // --- 數據觀看 (New) ---
  RACING_DATA_VIEW: 'racing:data_view', // 能否看到數據分頁與縱覽的訓練趨勢分析
} as const;
...
export const ROLE_PERMISSIONS: Record<string, string[]> = {
  ...
  [ROLES.COACH]: [
    ...
    PERMISSIONS.RACING_DATA_VIEW // COACH 自動擁有觀看數據權限
  ],
  ...
  [ROLES.AIDE]: [
    ...
    PERMISSIONS.RACE_MANAGE,
    PERMISSIONS.CHECK_IN,
    PERMISSIONS.RACING_DATA_VIEW // AIDE 自動擁有觀看數據權限
  ],

  [ROLES.RACING]: [
    PERMISSIONS.RACING_DATA_VIEW // RACING 選手擁有觀看數據權限
  ],

  [ROLES.RIDER]: [
    // RIDER 只有基本檢視權，隱藏數據分頁
  ]
};
