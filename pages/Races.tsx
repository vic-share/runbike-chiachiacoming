...
const Races: React.FC<RacesProps> = ({ people, raceGroups, refreshData, initialExpandedEventId, raceEvents = [] }) => {
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await api.manageRaceEvent('create', {
  ...
  const handleJoin = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await api.joinOrUpdateRace(
        Number(event.id),
        Number(targetPeopleId),
        joinForm.value,
        joinForm.race_group,
        joinForm.note,
        joinForm.photo_url || undefined
      );
      if (res.success) {
        refreshData();
  ...
  const confirmExitRace = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await api.exitRace(Number(eventId), Number(peopleId));
      if (res.success) {
        refreshData();
  ...
  const confirmDeleteRaceEvent = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await api.manageRaceEvent('delete', { id: Number(deleteConfirm.id) });
      if (res.success) {
        refreshData();
  ...
  const handleTogglePersonalHonor = async () => {
    ...
    try {
      ...
      const res = await api.joinOrUpdateRace(
        Number(event.id),
        Number(participant.people_id || participant.id),
        participant.value || '',
        participant.race_group || '',
        participant.note || '',
        participant.photo_url || undefined,
        nextStatus
      );
      if (res.success) {
        refreshData();
  ...
  const handleToggleGlobalHonor = async () => {
    ...
    try {
      ...
      const res = await api.setGlobalHonor(Number(participant.id || participant.people_id), duration);
      if (res.success) {
        refreshData();
  ...
  const handleParticipantUploadStart = (e: React.ChangeEvent<HTMLInputElement>, eventId?: string|number) => {
    const file = e.target.files?.[0];
    if (!file) return;
    setActiveUploadEventId(eventId || 'form');
  ...
  const handleCropSave = async () => {
    ...
    try {
      ...
      const folder = activeUploadEventId ? 'records' : 'races';
      const result = await uploadImage(file, folder);
      
      if (result.url) {
        if (!activeUploadEventId) {
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative w-full aspect-[4/5] max-h-48 rounded-2xl bg-zinc-900 border border-white/10 overflow-hidden group">
                    ...
                    <input type="file" accept="image/*" className="hidden" id="upload-participant-photo" onChange={(e) => handleParticipantUploadStart(e, showJoinModal.event?.id)} />
                  </div>
                </div>
