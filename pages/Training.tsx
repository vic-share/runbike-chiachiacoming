...
const Training: React.FC<any> = ({ trainingTypes, defaultType, refreshData, data, people, activePersonId, onSelectPerson, raceEvents, initialExpandedDate, onClearJumpDate }) => {
  ...
  useEffect(() => {
      if (initialExpandedDate) {
          ...
          setTimeout(() => {
              ...
              if (onClearJumpDate) onClearJumpDate();
          }, 800);
      }
  }, [initialExpandedDate]);
  ...
  const activeRider = useMemo(() => {
      ...
      return null;
  }, [filteredPeople, currentRiderId, activePersonId, isRecordingMode]);
  ...
  const calculateStability = (values: number[]) => {
      if (values.length <= 1) return values.length === 1 ? 100 : 0;
  ...
  const overviewRecords = useMemo(() => {
      ...
      return target.filter((d: any) => {
          ...
          if (dateRange === 'PICK') {
              ...
              return isValid(start) && isValid(end) ? isWithinInterval(dDate, { start: startOfDay(start), end: endOfDay(end) }) : true;
          }
  ...
  }, [data, activeRider, selectedType, dateRange, customDateRange]);
  ...
  const honorList = useMemo(() => {
      ...
      return raceEvents.filter((e: any) => {
          ...
          const p = e.participants.find((part: any) => String(part.people_id || part.id) === String(activeRider.id));
          return { id: e.id, date: e.date, name: e.name, rank: p.race_group || '完賽' };
      }).sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const handleRiderSelection = (id: string) => {
      if (isRecordingMode) {
          setParticipatingRiderIds(prev => { 
              if (prev.includes(id)) { 
                  const next = prev.filter(pid => pid !== id); 
                  if (currentRiderId === id) setCurrentRiderId(next.length > 0 ? next[0] : null); 
                  return next; 
              } else { 
                  const next = [...prev, id]; 
                  if (!currentRiderId) setCurrentRiderId(id); 
                  return next; 
              } 
          });
            ...
            <div className="glass-card w-full max-w-sm rounded-t-[32px] p-6 bg-zinc-950 border-chiachia-green/20 flex flex-col gap-4 animate-slide-up max-h-[85vh] overflow-hidden" onClick={e => e.stopPropagation()}> 
              ...
              <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-4 p-1"> 
                {sortedPeople.map((p: any) => { 
                  ...
                  return ( 
                    <button key={p.id} onClick={() => handleRiderSelection(String(p.id))} className={`relative flex flex-col items-center gap-3 p-4 rounded-3xl border transition-all ${isSelected ? 'bg-chiachia-green/10 border-chiachia-green' : 'bg-zinc-900 border-white/5'}`}> 
                      <div className={`w-20 h-20 rounded-full overflow-hidden border-2 bg-black ${isSelected ? 'border-chiachia-green shadow-glow-green' : 'border-zinc-700'}`}> {p.s_url ? <img src={p.s_url.split('#')[0]} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-zinc-600"><UserPlus size={28}/></div>} </div> 
                      <span className={`text-sm font-black truncate w-full text-center ${isSelected ? 'text-white' : 'text-zinc-500'}`}>{p.name}</span> 
                      {isSelected && <div className="absolute top-3 right-3 w-6 h-6 bg-chiachia-green rounded-full flex items-center justify-center text-black shadow-sm"><Check size={14} strokeWidth={4}/></div>} 
                    </button> 
                  ); 
                })} 
              </div> 
